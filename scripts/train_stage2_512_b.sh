...
# Stage 2: PointDiT-B fine-tuning at 512x512 on the 11-dataset mixture.
# Original run: 64 GPUs x batch 32 (effective batch 2048, lr 8e-4); released at epoch 7.
# lr = blr * batch_size * num_gpus / 256, so keep that product at 2048 or pass --lr.
...
STAGE1_CKPT=checkpoints/pointdit-b-256/checkpoint-last.pth
OUTPUT_DIR=checkpoints/pointdit-b-512
mkdir -p ${OUTPUT_DIR}
...
--batch_size 32 --blr 1e-4 \
--epochs 8 --warmup_epochs 0 \
